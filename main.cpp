...
void flow_poll() {
  ...
  if (flow_start == 0) {
    flow_gallons = 0;  // if first pulse, record time
    flow_start = curr;
  }
...
void do_loop()
{
  ...
  switch (os.state) {
    case OS_STATE_INITIAL:
      DEBUG_PRINTLN("OS INITIAL STATE");
      if (useEth) {
        led_blink_ms = 0;
        ...
        DEBUG_PRINTLN(os.get_wifi_mode());
        DEBUG_PRINTLN("NOT IN AP MODE");
...
void process_dynamic_events(ulong curr_time) {
  ...
  for (bid = 0; bid < os.nboards; bid++) {
    ...
    for (s = 0; s < 8; s++) {
      ...
      if (!en)	{
        q->deque_time = curr_time;  // if system is disabled, turn off zone
        turn_off_station(sid, curr_time);
      }
      if (rd && !(igrd & (1 << s))) {
        q->deque_time = curr_time;  // if rain delay is on and zone does not ignore rain delay, turn it off
        turn_off_station(sid, curr_time);
      }
      if (sn1 && !(igs & (1 << s))) {
        q->deque_time = curr_time;  // if sensor1 is on and zone does not ignore sensor1, turn it off
        turn_off_station(sid, curr_time);
      }
      if (sn2 && !(igs2 & (1 << s))) {
        q->deque_time = curr_time;  // if sensor2 is on and zone does not ignore sensor2, turn it off
        turn_off_station(sid, curr_time);
      }
    }
  }
...
void push_message(int type, uint32_t lval, float fval, const char* sval) {
  ...
  switch (type) {
    case  NOTIFY_STATION_ON:
      ...
      DEBUG_PRINTLN("Sending ON notification");
      if (os.mqtt.enabled()) {
    ...
    case NOTIFY_STATION_OFF:
      ...
      DEBUG_PRINTLN("Sending OFF notification");
      if (os.mqtt.enabled()) {
        sprintf_P(topic, PSTR("opensprinkler/station/%d"), lval);
        if (os.iopts[IOPT_SENSOR1_TYPE] == SENSOR_TYPE_FLOW) {
    ...
    case  NOTIFY_STATION_ALL:
      ...
      if (os.mqtt.enabled()) {
        ...
        sprintf_P(topic, PSTR("opensprinkler/station/all"), lval);
        sprintf_P(payload, PSTR("{\"states\":%s}"), (char)pinstatuses);
      }
